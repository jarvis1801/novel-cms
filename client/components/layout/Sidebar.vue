<template>
  <aside class="menu app-sidebar animated" :class="{ slideInLeft: show, slideOutLeft: !show }">
    ...
    <ul class="menu-list">
        ...
        <expanding v-if="item.children && item.children.length">
          <ul v-show="isExpanded(item)">
            <li v-for="subItem in item.children" :key="subItem.name">
              <router-link v-if="subItem.path" :to="generatePath(item, subItem)" v-bind:class="{'disable-click': getDisableClickList(subItem) }">
      ...
      <li>
        <router-link :to="'/'" :exact="true" >Logout</router-link>
      </li>
    </ul>
  </aside>
</template>
...
export default {
  ...
  methods: {
    ...
    generatePath (item, subItem) {
      return `${item.path ? item.path + '/' : ''}${subItem.path}`
    },
    ...
    getDisableClickList(subItem) {
      const disableClickList = ['Volume', 'Create Volume', 'Chapter', 'Create Chapter']
      if (_.includes(disableClickList, subItem.name)) {
        return true
      }
      return false
    }
